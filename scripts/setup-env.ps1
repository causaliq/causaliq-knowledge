# Comprehensive Python environment management for causaliq-knowledge
# Usage: .\scripts\setup-env.ps1 [options]

param(
    [switch]$Install,           # Install causaliq-knowledge package after creating environments
...
function Show-Help {
    ...
    Write-Host "  -Install         Create environments AND install causaliq-knowledge package" -ForegroundColor White
    Write-Host "  -InstallOnly     Install causaliq-knowledge package in existing environments" -ForegroundColor White
    Write-Host "  -Help            Show this help message" -ForegroundColor White
    Write-Host ""
    Write-Host "EXAMPLES:" -ForegroundColor Yellow
    Write-Host "  .\scripts\setup-env.ps1                    # Create environments only" -ForegroundColor Gray
    Write-Host "  .\scripts\setup-env.ps1 -Install           # Create + install causaliq-knowledge" -ForegroundColor Gray
    Write-Host "  .\scripts\setup-env.ps1 -InstallOnly       # Install causaliq-knowledge in existing envs" -ForegroundColor Gray
    Write-Host ""
...
if ($InstallOnly) {
    ...
    Write-Host "Installing causaliq-knowledge in all environments..." -ForegroundColor Blue
    ...
    if ($Install) {
        Write-Host "(Will also install causaliq-knowledge package)" -ForegroundColor Gray
    }
...
if (-not ($Install -or $InstallOnly)) {
    Write-Host "To install causaliq-knowledge in all environments:" -ForegroundColor White
